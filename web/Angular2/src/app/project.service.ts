...
export class ProjectService {
    private baseUrl = 'http://bucketlist.loc/app_dev.php/api/v1.0/';
    private goalUrl = '';  // URL to web API
    private usersUrl = 'users';  // URL to web API
    private userGoalsUrl = 'usergoals';  // URL to web API
    private discoverGoalsUrl = this.baseUrl + 'discover-goals';  // URL to web API
    ...
    //  * @returns {Observable<R>}
    //  */
    // addChecklist(checklist:User):Observable<User> {
    //
    //     let headers = new Headers({'Content-Type': 'application/x-www-form-urlencoded'});
    //
    //     return this.http.put(this.checklistUrl, checklist.getUrlEncodedBody(), {headers: headers})
    //         .map((r:Response) => r.json() as User)
    //         .catch(this.handleError);
    // }
    ...
    //     let headers = new Headers({'Content-Type': 'application/x-www-form-urlencoded'});
    //
    //     return this.http.put(this.taskUrl, task.getUrlEncodedBody(), {headers: headers})
