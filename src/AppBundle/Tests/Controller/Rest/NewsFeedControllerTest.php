...
class NewsFeedControllerTest extends BaseClass
{
    ...
    public function testGetActivity()
    {
        ...
        //get success story data in array
        ...
        $responseResultStory = $responseResults[1];

        if(array_key_exists('goal', $responseResultStory)) {
            $goal = $responseResultStory['goal'];
            $this->assertContains('goal13', $goal, 'Invalid goal13 value in news-feed getAction rest json structure');
        }

        if(array_key_exists('success_story', $responseResultStory)) {
            $successStory = $responseResultStory['success_story'];
        ...
        //get comment data in array
        $responseResultComment = $responseResults[12];

        if(array_key_exists('goal', $responseResultComment)) {
            $goal = $responseResultComment['goal'];
            $this->assertContains('goal13', $goal, 'Invalid goal13 value in news-feed getAction rest json structure');
        }

        if(array_key_exists('comment', $responseResultComment)) {
            $comment = $responseResultComment['comment'];
